...
@file: debert_dataloader.py
@time: 2022/3/22 21:35
@desc:
'''
import os
...
class DataMaker(object):
    def __init__(self, tokenizer, add_special_tokens=True, define_type='idiom'):
        ...
        if define_type == 'idiom':
            definition_f = open('/home/yukino/Downloads/NLP/CIMD/DEBERT/Idiom/Idiom_Definition.json', 'r', encoding='utf-8')
        else:
            definition_f = open('../xiandao/Word_Definition', 'r', encoding='utf-8')
        lines = definition_f.readlines()
        self.definitions = {}
        self.sentiment_analyzer = SentimentAnalyzer(device='gpu')
    ...
    def generate_inputs(self, datas, max_seq_len, ent2id):
        ...
        for sample in tqdm(datas, desc='in generate inputs function : '):
            ...
            # 计算情感特征
            try:
                text_sentiment = float(self.sentiment_analyzer.get_sentiment_score(sample['text']))
            except:
                text_sentiment = 0.0

            definition_sentiments = []
            for worditem in sample['entity_list']:
                word = sample["text"][worditem[0]:worditem[1]+1]
                definition = self.definitions.get(word, {'definition': '[PAD]'})['definition']
                try:
                    if definition != '[PAD]':
                        def_sentiment = float(self.sentiment_analyzer.get_sentiment_score(definition))
                    else:
                        def_sentiment = 0.0
                except:
                    def_sentiment = 0.0
                definition_sentiments.append(def_sentiment)
            ...
            sample_input = {
                ...
                'definition_sentiments': definition_sentiments
            }
    ...
    def generate_batch(self, batch_data, data_type="train"):
        ...
        text_sentiment = [batch['text_sentiment'] for batch in batch_data]
        definition_sentiments = [batch['definition_sentiments'] for batch in batch_data]

        # Get maximum number of definitions in the batch
        max_definitions = max(len(sents) for sents in definition_sentiments)
        # Pad shorter sequences with 0.0
        definition_sentiments = [sents + [0.0] * (max_definitions - len(sents)) for sents in definition_sentiments]
        ...
        text_sentiment = torch.tensor(text_sentiment).float()
        definition_sentiments = torch.tensor(definition_sentiments).float()
        
        return (input_ids, attention_mask, token_type_ids, define_ids, define_type_ids, define_cls, 
                match_word_ids, labels, text_sentiment, definition_sentiments)
