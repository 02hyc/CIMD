...
from configs_improved import config
...
from GlobalPointer import MetricsCalculator
from DirectOutputModel import DirectOutputModel
...
if config["run_type"] == "train" and not os.path.exists(model_state_dict_dir):
    ...
    print('DEBERT训练程序, 使用可训练alpha, 改进版本')
...
def data_generator(run_type='train', cache_path='/home/yukino/Documents/Code/NLP/CIMD/DEBERT/data_cache.pt', use_cache=False):
...
def train_step(batch_train, model, optimizer, criterion, scaler, get_metric=False):
    ...
    if config['use_half']:
        ...
        # 使用配置中的梯度裁剪值
        torch.nn.utils.clip_grad_norm_(model.parameters(), config.get('gradient_clip_val', 5.0))
...
def train(model, dataloader, epoch, optimizer, scheduler, scaler):
    ...
    for batch_ind, batch_data in tqdm(enumerate(dataloader)):
        ...
        # 根据scheduler类型决定是否在每个batch后更新学习率
        if config['scheduler'] != 'CosW':
            scheduler.step()
...
if __name__ == '__main__':
    ...
    if 'dropout_rate' in config:
        bertconfig.hidden_dropout_prob = config['dropout_rate']
        bertconfig.attention_probs_dropout_prob = config['dropout_rate']
    ...
    optimizer = torch.optim.AdamW(
        ...
        weight_decay=config.get('weight_decay', 0.01)  # 使用权重衰减
    )
    ...
    if config["scheduler"] == "CAWR":
        ...
        import math
        total_train_steps = len(train_dataloader) * config['epochs']
        num_warmup_steps = int(total_train_steps * 0.02)
    ...
    for epoch in range(config["epochs"]):
        avg_loss = train(model, train_dataloader, epoch, optimizer, scheduler, scaler)
        ...
        # 检查早停条件
        if early_stopping and early_stopping(valid_f1_sim):
